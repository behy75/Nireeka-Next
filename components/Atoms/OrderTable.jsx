...
import { setSelectedOrderId } from "../../app/userPanelSlice";
...
export default function OrderTable({
  ...
  let { ordersData, selectedOrderId } = state.userPanel;

  useEffect(() => {
    if (selectedOrderId === 0) {
      dispatch(setSelectedOrderId(ordersData.order_bikes[0].frame_no));
    }
  ...
  return (
    <div
      className={`w-full bg-white rounded-3xl my-2 shadow-sm border ${
        selectedOrderId === frameNumber ? "border-blue-700" : "border-gray-200"
      } cursor-pointer`}
      onClick={() => dispatch(setSelectedOrderId(frameNumber))}
    >
